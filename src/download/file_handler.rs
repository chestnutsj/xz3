...
pub async fn checkout_filename(
    output: PathBuf,
    status_file: PathBuf,
    ...
    match fs::metadata(&output).await {
        Ok(_) => match fs::metadata(status_file.clone()).await {
            Ok(_) => {
                        ...
                        match old_task {
                            Ok(t) => {
                                if t.url == task_info.url {
                                    ...
                                    let new_output =
                                        generate_unique_filename(&output, extension, 1).await;
                                    output = new_output;
                                }
                            }
                            Err(_) => {
    ...
    match general_purpose::STANDARD.decode(content_md5_base64) {
        Ok(decoded_md5) => {
            if decoded_md5 == local_md5 {
                ...
                Err(anyhow!(
                    "Content-MD5 verification failed {:?} {:?}",
                    decoded_md5,
                    local_md5
                ))
            }
