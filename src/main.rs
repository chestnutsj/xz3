...
async fn main() -> Result<()> {
     ...
     if let Some(logfile) = args.log.clone() {
        // 配置文件输出
         let file_appender = FileAppender::builder()
            ...
            log_config = log_config.appender(Appender::builder().filter(Box::new(ThresholdFilter::new(LevelFilter::Info))).build("logfile", Box::new(file_appender)));
            log_build = log_build.appender("logfile");
    } 

    let configbuild = log_config.build(
        log_build
             .build(log_level),
     ).unwrap();
    ...
    task::start_single_task(&args.url,&file_name,  args.threadsize , None, Some(args.max_retry)).await?;
